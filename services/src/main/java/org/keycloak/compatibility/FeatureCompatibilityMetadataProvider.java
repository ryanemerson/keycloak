...
public class FeatureCompatibilityMetadataProvider implements CompatibilityMetadataProvider {
    ...
    public Map<String, String> metadata() {
        ...
        for (Profile.Feature f : features) {
            ...
            metadata.compute(feature.key, (k, v) -> {
                if (v == null) {
                    return toJson(feature);
                }
                Feature existing = fromJson(v);
                if (feature.version > existing.version)
                    return toJson(feature);
                return v;
            });
        }
    ...
    record Feature(String key, boolean enabled, int version, Profile.FeatureUpdatePolicy updatePolicy) {
        static Feature from(Profile.Feature feature) {
            return new Feature(
                  feature.getUnversionedKey(),
