...
This is a building block following the concepts described in the <@links.ha id="multi-cluster-concepts-multi-cluster" /> {section}.
...
After the Aurora AWS database and {jdgserver_name} are deployed and running, use the procedure in the <@links.ha id="multi-cluster-deploy-keycloak-kubernetes" /> {section} to deploy {project_name} and connect it to all previously created building blocks.
