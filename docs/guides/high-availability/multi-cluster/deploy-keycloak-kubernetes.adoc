...
These instructions are intended for use with the setup described in the <@links.ha id="multi-cluster-concepts-multi-cluster"/> {section}.
Use it together with the other building blocks outlined in the <@links.ha id="multi-cluster-bblocks-multi-cluster"/> {section}.
...
* AWS Aurora database deployed using the <@links.ha id="multi-cluster-deploy-aurora-multi-az" /> {section}.
...
. Notice the configuration file below contains options relevant for connecting to the Aurora database from <@links.ha id="multi-cluster-deploy-aurora-multi-az" anchor="multi-cluster-aurora-connecting" />
