[#${parent}-threads-concept]
== Concepts

[#${parent}-jgroups-communications]
...
{project_name} allows configuring the maximum thread pool size by the link:{links_server_all-config_url}?q=http-pool-max-threads[`http-pool-max-threads`] configuration option. See <@links.ha id="multi-cluster-deploy-keycloak-kubernetes" /> for an example.
