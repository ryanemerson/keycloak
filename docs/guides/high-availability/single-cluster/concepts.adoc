...
Use this setup to provide {project_name} deployments that are contained within a single Kubernetes cluster.
...
| If the connectivity between Kubernetes nodes is lost, data cannot be sent between {jdgserver_name} pods hosted on those nodes.
Incoming requests might receive an error message or be delayed for some seconds.
The {jdgserver_name} will eventually remove the unreachable pods from its local view and will stop sending data to them.
| No data loss
| Seconds to minutes
...
| Connectivity {jdgserver_name}
| If the connectivity between nodes or availability-zones is lost, data cannot be sent between {jdgserver_name} pods hosted in those nodes or zones.
Incoming requests might receive an error message or be delayed for some seconds.
The {jdgserver_name} will eventually remove the unreachable pods from its local view and will stop sending data to them.
| No data loss
| Seconds to minutes
...
and `actionTokens` caches if the number of node failures is greater than the cache's configured `num_owners`.
+
