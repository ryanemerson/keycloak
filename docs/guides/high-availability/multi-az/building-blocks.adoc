...
== Two availability-zones with low-latency connection

Ensures that synchronous replication is available for both the database and {project_name} clustering.

*Suggested setup:* Two or more AWS Availability Zones within the same AWS Region.
...
*Suggested setup:* Red Hat OpenShift Service on AWS (ROSA) deployed across multiple availability-zones with at least one worker pool in each zone.
