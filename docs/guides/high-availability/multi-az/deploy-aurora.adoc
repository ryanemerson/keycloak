...
<#import "../partials/aurora/aurora-single-site.adoc" as aurora>
...
This deployment is intended to be used with the setup described in the <@links.ha id="multi-az-concepts"/> {section}.
Use this deployment with the other building blocks outlined in the <@links.ha id="multi-az-building-blocks"/> {section}.

include::../partials/blueprint-disclaimer.adoc[]

<@aurora.single_site "multi-az" />

[#connecting-aurora-to-multi-az-keycloak]
== Connecting Aurora database with {project_name}

Now that an Aurora database has been established and linked with all of your ROSA clusters, here are the relevant {project_name} CR options to connect the Aurora database with {project_name}. These changes will be required in the <@links.ha id="multi-az-deploy-keycloak-kubernetes" /> {section}. The JDBC url is configured to use the Aurora database writer endpoint.
...
After successful deployment of the Aurora database continue with <@links.ha id="multi-az-deploy-infinispan-kubernetes" />
