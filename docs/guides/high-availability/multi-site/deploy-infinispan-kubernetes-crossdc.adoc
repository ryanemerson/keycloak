...
Now that the {jdgserver_name} server is running, here are the relevant {project_name} CR changes necessary to connect it to {project_name}. These changes will be required in the <@links.ha id="multi-sitedeploy-keycloak-kubernetes" /> {section}.
...
[NOTE]
====
All the memory, resource and database configurations are skipped from the CR below as they have been described in the <@links.ha id="multi-sitedeploy-keycloak-kubernetes" /> {section} already.
...
After the Aurora AWS database and {jdgserver_name} are deployed and running, use the procedure in the <@links.ha id="multi-sitedeploy-keycloak-kubernetes" /> {section} to deploy {project_name} and connect it to all previously created building blocks.
