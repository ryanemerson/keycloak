...
These instructions are intended for use with the setup described in the <@links.ha id="multi-site/concepts-multi-site"/> {section}.
Use it together with the other building blocks outlined in the <@links.ha id="multi-site/bblocks-multi-site"/> {section}.
...
* Aurora AWS database deployed using the <@links.ha id="multi-sitedeploy-aurora-multi-az" /> {section}.
* {jdgserver_name} server deployed using the <@links.ha id="multi-sitedeploy-infinispan-kubernetes-crossdc" /> {section}.
...
. Determine the sizing of the deployment using the <@links.ha id="multisiteconcepts-memory-and-cpu-sizing" /> {section}.

. Install the {project_name} Operator as described in the <@links.operator id="installation" /> {section}.

. Notice the configuration file below contains options relevant for connecting to the Aurora database from <@links.ha id="multi-sitedeploy-aurora-multi-az" anchor="connecting-aurora-to-keycloak" />

. Notice the configuration file below options relevant for connecting to the {jdgserver_name} server from <@links.ha id="multi-sitedeploy-infinispan-kubernetes-crossdc" anchor="connecting-infinispan-to-keycloak" />
...
See the <@links.ha id="multisiteconcepts-database-connections" /> {section} for details.
...
See the <@links.ha id="multisiteconcepts-threads" /> {section} about load shedding for details.
