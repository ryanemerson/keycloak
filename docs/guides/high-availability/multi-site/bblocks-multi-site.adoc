...
* Understanding the concepts laid out in the <@links.ha id="multi-siteconcepts-multi-site"/> {section}.
...
*Blueprint:* <@links.ha id="multi-sitedeploy-aurora-multi-az"/>.

== {jdgserver_name}

A deployment of {jdgserver_name} that leverages the {jdgserver_name}'s Cross-DC functionality.

*Blueprint:* <@links.ha id="multi-sitedeploy-infinispan-kubernetes-crossdc" /> using the {jdgserver_name} Operator, and connect the two sites using {jdgserver_name}'s Gossip Router.
...
*Blueprint:* <@links.ha id="multi-sitedeploy-keycloak-kubernetes" /> that includes connecting to the Aurora database and the {jdgserver_name} server.
...
*Blueprint:* <@links.ha id="multi-sitedeploy-aws-accelerator-loadbalancer"/> together with  <@links.ha id="multi-sitedeploy-aws-accelerator-fencing-lambda"/>.
