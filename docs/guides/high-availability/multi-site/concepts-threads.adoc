...
For a configuration where this is applied, visit <@links.ha id="multi-sitedeploy-keycloak-kubernetes" />.
...
{project_name} allows configuring the maximum thread pool size by the link:{links_server_all-config_url}?q=http-pool-max-threads[`http-pool-max-threads`] configuration option. See <@links.ha id="multi-sitedeploy-keycloak-kubernetes" /> for an example.
