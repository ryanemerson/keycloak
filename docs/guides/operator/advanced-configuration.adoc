...
=== ServiceMonitor
The {project_name} operator automatically generates a `ServiceMonitor` resource if the `monitoring.coreos.com/v1:ServiceMonitor`
CRD is available on the {kubernetes} cluster. The spec used is as follows:
